...
## 3. Istio Ambient + Kiali 경로

- [ ] Gateway API CRD 설치 확인
- [ ] Istio Helm chart 기반 ambient 설치 성공
- [ ] `istiod` Ready
- [ ] `istio-cni-node` Ready
- [ ] `ztunnel` Ready
- [ ] Prometheus Ready
- [ ] Kiali Operator Ready
- [ ] Kiali Ready
...
- [ ] Kiali graph에서 smoke workload traffic 확인

## 4. Kiali 뒤 DeepFlow 추가
...
- [ ] Grafana 접속 경로 확인
- [ ] Kiali graph와 DeepFlow Service Map 결과 비교
- [ ] waypoint 적용 전/후 DeepFlow L7 복원 범위 비교

## 5. Smoke web-was-db

- [ ] `sidecarless-smoke` namespace 생성
- [ ] DeepFlow 단독 경로에서는 ambient label 제거 또는 무시 기준 확인
- [ ] Istio Ambient + Kiali 경로에서는 namespace ambient label 적용
...
- [ ] DeepFlow 단독 경로에서는 DeepFlow에서 HTTP/SQL 흐름 확인
- [ ] Istio Ambient + Kiali 경로에서는 Kiali에서 workload graph 확인
- [ ] DeepFlow 추가 경로에서는 Grafana에서 source/destination service 관계 확인
...
- [ ] 선택한 경로에 맞춰 Kiali 또는 DeepFlow 확인 절차 추가
...
- [ ] Helm provider로 Kiali 관리 가능성 확인
...
- [ ] Kiali graph와 DeepFlow Service Map 차이 정리
- [ ] NKS 적용 제약사항 정리
